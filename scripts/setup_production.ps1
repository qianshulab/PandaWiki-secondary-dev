...
if ($Auto) {
  ...
  $redisPassword = New-SafeSecret 32
  ...
  $redisPassword = Read-RequiredSecret "Redis 密码 REDIS_PASSWORD" 16
...
# Container network subnet prefix
...
if ($shouldStart) {
  ...
  Write-Host ""
  Write-Host "SUCCESS  控制台信息:" -ForegroundColor Green
  Write-Host "SUCCESS    访问地址(内网): https://服务器IP:2443" -ForegroundColor Green
  Write-Host "SUCCESS    访问地址(外网): https://服务器IP:2443" -ForegroundColor Green
  Write-Host "SUCCESS    用户名: admin" -ForegroundColor Green
  Write-Host "SUCCESS    密码: $adminPassword" -ForegroundColor Green
} else {
  Write-Host ""
