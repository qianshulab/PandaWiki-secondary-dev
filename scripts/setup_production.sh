...
JWT_SECRET="$(generate_secret 64)"
  ...
  REDIS_PASSWORD="$(read_required_secret "Redis 密码 REDIS_PASSWORD" 16)"
...
if [[ "$SHOULD_START" == "true" ]]; then
  ...
  echo
  echo "SUCCESS  控制台信息:"
  echo "SUCCESS    访问地址(内网): https://服务器IP:2443"
  echo "SUCCESS    访问地址(外网): https://服务器IP:2443"
  echo "SUCCESS    用户名: admin"
  echo "SUCCESS    密码: $ADMIN_PASSWORD"
else
  echo
  echo "已跳过启动。后续可执行："
